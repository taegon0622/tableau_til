...
```
먼저 국가/지역을 필터에 드래그하여 south korea로 필터링한다. 그리고 매출은 색상과 크기에 드래그하고, 하위범주는 레이블에 드래그 해준다. 그리고 매출을 레이블에 드래그 해준 뒤 퀵테이블 계산을 선택하여 구성비율로 변경해준다
...
그리고 필터에 입력한 국가 / 지역을 열에 입력하여 표기한다
```
